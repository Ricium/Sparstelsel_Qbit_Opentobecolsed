<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<CashierInstantMoney>" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <script>
        ...
        function buttonInsertFunds() {
            ...
            $.ajax({
                type: 'POST',
                url: '/Money/_InsertElectronicFromMultiInstantMoney/',
                data: form,
                dataType: 'html',
        ...
        function buttonInsertInstantMoneys() {
            var form = $('#InstantMoneyForm').serialize();

            $.ajax({
                type: 'POST',
                url: '/Money/_InsertInstantMoneyFromMultiInstantMoney/',
                data: form,
                dataType: 'html',
                success: function (htmlData) {
        ...
        function InsertInstantMoneys(e) {
            var charCode = (typeof e.which === "number") ? e.which : e.keyCode;
        ...
        function ChangeDate() {
            ...
            $('#InstantMoney_ActualDate').val(new_val);
        ...
        function ChangeEmployee() {
            ...
            $('#InstantMoney_EmployeeID').val(employeepicker.value());
        ...
        function GotoReport() {
            ...
            $.post('/Money/_CashierInstantMoneyReport/', form, function (data) {
                $('#Report').html(data);
            });
        }
    ...
    <div id="CashContainer" style="font-size: large"></div>                      
              ...
              tab.Add().Text("InstantMoney").Content(() =>
                          { %>
    <table>
        <tr>
            <td>
                <form id="InstantMoneyForm">
                    <table>
                        <tr>
                            <td>
                                <%: Html.HiddenFor(m => m.InstantMoney.InstantMoneyID)%>
                                <%: Html.HiddenFor(m => m.InstantMoney.ActualDate) %>
                                <%: Html.HiddenFor(m => m.InstantMoney.EmployeeID) %>                                
                        ...
                        <tr>
                            <td>
                                <%: Html.LabelFor(m => m.InstantMoney.InstantMoneyTypeID)%>
                            </td>
                            <td>
                                <%: Html.Telerik().DropDownListFor(m => m.InstantMoney.InstantMoneyTypeID).BindTo((IEnumerable<SelectListItem>)ViewData["InstantMoneyType"]).HtmlAttributes(new { style = "width: 250px" })%>
                                <%: Html.ValidationMessageFor(model => model.InstantMoney.InstantMoneyTypeID)%>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <%: Html.LabelFor(m => m.InstantMoney.Amount)%>
                            </td>
                            <td>
                                <%: Html.TextBoxFor(m => m.InstantMoney.Amount, new { onkeyup = "InsertInstantMoneys(event)" })%>
                                <%: Html.ValidationMessageFor(m => m.InstantMoney.Amount)%>
                            </td>
                        </tr>
                    </table>
                </form>
          <button id="Save" onclick="Save()" class="t-button">Save</button>
                <button type='button' class='t-button' id='InsertInstantMoneys' onclick='buttonInsertInstantMoneys()'>Insert</button>
            </td>
            <td>
