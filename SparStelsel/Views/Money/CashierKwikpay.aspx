<%@ Page Title="" Language="C#" MasterPageFile="~/Views/Shared/Site.Master" Inherits="System.Web.Mvc.ViewPage<CashierKwikpay>" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <script>
        ...
        function buttonInsertFunds() {
            ...
            $.ajax({
                type: 'POST',
                url: '/Money/_InsertElectronicFromMultiKwikpay/',
                data: form,
                dataType: 'html',
        ...
        function buttonInsertKwikpays() {
            var form = $('#KwikpayForm').serialize();

            $.ajax({
                type: 'POST',
                url: '/Money/_InsertKwikpayFromMultiKwikpay/',
                data: form,
                dataType: 'html',
                success: function (htmlData) {
        ...
        function ChangeDate() {
            ...
            $('#KiwkPay_ActualDate').val(new_val);
        ...
        function ChangeEmployee() {
            ...
            $('#KiwkPay_EmployeeID').val(employeepicker.value());
        ...
        function GotoReport() {
            ...
            $.post('/Money/_CashierKwikpayReportShow/', form, function (data) {
                $('#Report').html(data);
            });
        }
            ...
            <td>
                <form id="KwikpayForm">
                    <table>
                        <tr>
                            <td>
                                <%: Html.HiddenFor(m => m.KiwkPay.KwikPayID)%>
                                <%: Html.HiddenFor(m => m.KiwkPay.ActualDate) %>
                                <%: Html.HiddenFor(m => m.KiwkPay.EmployeeID) %>                                
                        ...
                        <tr>
                            <td>
                                <%: Html.LabelFor(m => m.KiwkPay.KwikPayTypeID)%>
                            </td>
                            <td>
                                <%: Html.Telerik().DropDownListFor(m => m.KiwkPay.KwikPayTypeID).BindTo((IEnumerable<SelectListItem>)ViewData["KwikPayType"]).HtmlAttributes(new { style = "width: 250px" })%>
                                <%: Html.ValidationMessageFor(model => model.KiwkPay.KwikPayTypeID)%>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <%: Html.LabelFor(m => m.KiwkPay.Amount)%>
                            </td>
                            <td>
                                <%: Html.TextBoxFor(m => m.KiwkPay.Amount, new { onkeyup = "InsertKwikpays(event)" })%>
                                <%: Html.ValidationMessageFor(m => m.KiwkPay.Amount)%>
                            </td>
                        </tr>
                    </table>
                </form>
                <button type='button' class='t-button' id='InsertKwikpays' onclick='buttonInsertKwikpays()'>Insert</button>
                <br />
                <br />
